...
<details>
  ...
  async def control_runner(device_type, device_ip, device_id, device_key, token) :
      ...
      async with SwitcherType2Api(device_type, device_ip, device_id, device_key, token) as api:
          ...
          await api.api.set_light(LightState.ON, 0)
          # turn off the light, number id is 0 (Only for Runner S11 and Runner S12)
          await api.api.set_light(LightState.OFF, 0)
...
- [validate_token.py](https://github.com/TomerFi/aioswitcher/blob/dev/scripts/validate_token.py) can validate a device token which is a must for newer devices, used for communicating with devices.
