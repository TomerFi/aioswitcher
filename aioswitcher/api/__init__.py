...
from asyncio import (AbstractEventLoop, BaseTransport, Event,  # noqa F401
                     open_connection, sleep, StreamReader, StreamWriter)
...
from ..protocols import SwitcherV2TcpProtocolFactory
...
class SwitcherV2ApiLowTest:
    ...
    def __init__(self, loop: AbstractEventLoop, ip_addr: str, phone_id: str,
        ...
        self._protocol = None  # type: Optional[SwitcherV2TcpProtocolFactory]

    async def __aenter__(self) -> 'SwitcherV2ApiLowTest':
        """Enter context manager."""
        await self.connect()
        return await self.__await__()

    async def __await__(self) -> 'SwitcherV2ApiLowTest':
    ...
    async def connect(self) -> None:
        """Connect to socket."""
        self._transport, protocol_temp = await self._loop.create_connection(
            partial(SwitcherV2TcpProtocolFactory, *[self._loop]),
            host=self._ip_addr,
            port=SOCKET_PORT,
            family=AF_INET)

        self._protocol = cast(SwitcherV2TcpProtocolFactory, protocol_temp)
    ...
    async def _full_login(self) \
        ...
        if self._protocol:
            await self._protocol.send_packet(unhexlify(signed_packet))

            return timestamp, messages.SwitcherV2LoginResponseMSG(
                self._loop, self._protocol.response)
    ...
    async def _full_get_state(self) \
        ...
        if login_response and login_response.successful:
            ...
            if self._protocol:
                ...
                state_response = await messages.SwitcherV2StateResponseMSG(
                    self._loop, self._protocol.response).init_future.result()
